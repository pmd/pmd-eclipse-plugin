...
/**
 * A SWT-Composite for showing a DataflowGraph as well as other information in
 * form of a table
 *
...
public class DataflowGraphTable extends Composite implements PaintListener {
     ...
     * @param titles,
     *            String-Field with the Titles
     * @param graphPos,
     *            Number of Column, where to show the Graph
     */
    ...
    protected void buildFrameLines(GC g) {
        ...
        if (viewWidth > tableSize.x) {
            for (int i = 0; i < numRows; i++) {
                g.drawLine(tableSize.x, tableY + i * rowHeight.intValue(), viewWidth,
    ...
    private void buildTableData(Composite table, List<List<DataflowGraphTableData>> tableData) {
        ...
        for (int i = 0; i < tableData.size(); i++) {
            ...
            for (int j = 0; j < numCols; j++) {
                ...
                createLabel(table, style, text, bgColor, fgColor, new Point(xPos, i * rowHeight.intValue()),
     ...
     */
    protected void createLabel(Composite parent, int style, String text, Color bgColor, Color fgColor, Point coord,
            Point size) {
